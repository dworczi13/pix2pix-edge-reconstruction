#Dynamiczny rozmiar wejścia generatora: Model generatora akceptuje teraz obrazy o dowolnym rozmiarze (Input(shape=[None, None, 3])), co zwiększa jego elastyczność.
#Augmentacja danych w locie: Dodano nową funkcję get_augmented_pair, która dynamicznie przetwarza obrazy z zestawu danych. Wykonuje ona losowe przycinanie obrazów do rozmiaru 128x128 pikseli oraz losowe odbicia lustrzane (flip).
#Ręczne tworzenie paczek (batching): W pętli treningowej fit paczki danych (batche) są tworzone ręcznie poprzez wielokrotne wywołanie funkcji get_augmented_pair i składanie wyników w tensory.
#Stały rozmiar wejścia dyskryminatora: Dyskryminator został zdefiniowany tak, aby przyjmować obrazy o stałym rozmiarze 128x128, co jest spójne z nowym sposobem augmentacji danych.
#Trening na fragmentach obrazów: Model jest teraz trenowany na losowych wycinkach (patchach) 128x128 z oryginalnych obrazów, a nie na całych przeskalowanych obrazach.
...
exp_dir = f"exp_{script_name.replace('_3_tgd','')}"
...
# Funkcja augmentacji (opcjonalnie)
...
input_train_dir = pathlib.Path("/mnt/home/datasets/coco256/coco256_gt_in/train_30000_gt_in_qp47")
output_train_dir = pathlib.Path("/mnt/home/datasets/coco256/coco256_gt_out/train_30000_gt_out")
...
# Analogicznie dla testów (jeśli masz testowe PNG)
#input_test_dir = pathlib.Path("./datasets/facades/facades_in/test_106_in/")
#output_test_dir = pathlib.Path("./datasets/facades/facades_out/test_106_out/")
#input_test_paths = sorted(input_test_dir.glob('*.png'))
#output_test_paths = sorted(output_test_dir.glob('*.png'))
#input_test_files = tf.data.Dataset.from_tensor_slices([str(p) for p in input_test_paths])
#output_test_files = tf.data.Dataset.from_tensor_slices([str(p) for p in output_test_paths])
#paired_test_files = tf.data.Dataset.zip((input_test_files, output_test_files))
#test_dataset = paired_test_files.map(load_and_nor_image_for_training, num_parallel_calls=tf.data.AUTOTUNE)
...
def generate_images(model, test_input, tar, filename=None):
      # Dodaj wymiar batcha jeśli go brakuje
  ...
  for i in range(3):
    ...
    plt.axis('off')
  # Zapisuj obraz tylko jeśli filename wskazuje na katalog postępu
...
def get_augmented_pair(train_iterator):
    ...
    #input = input[0] if len(input.shape) == 4 else input
    #utput = output[0] if len(output.shape) == 4 else output
    # Losowy crop 128x128
...
def fit(train_ds, steps):
    ...
    # Przygotowanie pliku do logowania strat
    ...
    while step < exp_steps:
        ...
        step += 1
        #del input_batch, output_batch  # Zwolnij pamięć
        ...
        if (step + 1) % save_weights_every_n_steps == 0:
            ...
            # Dopisz wartości funkcji strat do pliku CSV
